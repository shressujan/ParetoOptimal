...
csv_file = os.path.join('cleaned_csv_files', name_only + '.csv')
...
def pareto_frontier(solutions, Xs, Ys, maxX=True, maxY=True):
    # Sort the list in either ascending or descending order of X
    myList = sorted([[Xs[i], Ys[i], solutions[i]] for i in range(len(Xs))], reverse=maxX)
    ...
    for pair in myList[1:]:
        if maxY:
            ...
            if pair[1] <= p_front[-1][1]:  # Look for lower values of Y…
                p_front.append(pair)  # … and add them to the Pareto frontier
                pf_set.add(pair[2])   # add the pareto frontier to the pf_set
...
read_data = pd.read_csv(csv_file, header=None, names=['Name', 'Table_Number', 'UnionSuperclassStrategy', 'UnionSubclassStrategy', 'JoinedSubclassStrategy', 'ForeignKeyEmbeddingStrategy', 'OwnAssociationTableStrategy', 'InsertTime', 'SelectTime', 'Average Time', 'SpaceConsumption'])

# Plot 2D Pareto Optimal graphs
table1 = read_data[['Name', 'InsertTime', 'SpaceConsumption']]
table2 = read_data[['Name', 'InsertTime', 'SelectTime']]
table3 = read_data[['Name', 'SelectTime', 'SpaceConsumption']]
# get your data from somewhere to go here
solutions = table1.Name
...
p_front = pareto_frontier(solutions, Xs, Ys, maxX=False, maxY=False)
# Plot a scatter graph of all results
plt.figure()
plt.scatter(Xs, Ys, s=normal_dot_size, color='black', label="All Generated Solutions")
# Then plot the Pareto frontier on top
plt.plot(p_front[0], p_front[1], mfc='white', linewidth=linewidth, color='black', markeredgewidth=1.5, marker='^',
         markersize=marker_size, label='Pareto Frontier')
...
# Adding new column to the csv file with pareto_frontiers distinction
if not os.path.exists('ground_truth'):
    os.mkdir('ground_truth')

solutions = read_data['Name']
ground_truth = [1 if name in pf_set else 0 for name in solutions]
read_data['Pareto_Frontier'] = ground_truth

ground_truth_file = os.path.join('ground_truth', file_name)
read_data.to_csv(ground_truth_file, index=False)
